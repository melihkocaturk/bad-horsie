        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <div class="p-4 sm:p-8 bg-white shadow sm:rounded-lg">
                <div class="max-w-none">
                    <section>
                        <x-link-button :href="route('my-horses.create')">
                            Add New Horse
                        </x-link-button>

                        @if (count($myHorses) > 0)
                            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                                @foreach ($myHorses as $myHorse)
                                    <a class="flex flex-wrap flex-col mt-6 bg-white border shadow-sm rounded-lg hover:shadow-lg transition dark:bg-gray-800 dark:border-gray-700 dark:shadow-slate-700/[.7]"
                                        href="{{ route('my-horses.show', $myHorse) }}">
                                        <img class="w-full h-auto rounded-t-lg"
                                            src="{{ asset('storage/' . $myHorse->avatar) }}" alt="{{ $myHorse->name }}">
